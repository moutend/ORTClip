    ...
    connection.on('message', function (message) {
      if (message.type === 'utf8') {
        var _ret = (function () {
          ...
          if (CODE === 'SET') {
            var QUERY = ['UPDATE test_table SET', 'isUsing = True,', 'hash = $$' + HASH + '$$,', 'timestamp = to_timestamp(' + TIMESTAMP + '),', 'message = $$' + MESSAGE + '$$', 'WHERE id = (', '  SELECT id FROM test_table', '  WHERE isUsing = False', '  OR timestamp < to_timestamp(' + (TIMESTAMP - 90) + ') LIMIT 1', ')'].join(' ');
            ...
            sendSQL(QUERY).then(function (result) {
              ...
              var Q = ['SELECT id FROM test_table', 'WHERE hash = $$' + HASH + '$$'].join(' ');
