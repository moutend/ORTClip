    ...
    connection.on('message', function (message) {
      if (message.type === 'utf8') {
        var _ret = (function () {
          ...
          if (CODE === 'SET') {
            var UPDATE_ROW = ['UPDATE ' + TABLE_NAME + ' SET', 'isUsing = True,', 'hash = $$' + HASH + '$$,', 'timestamp = to_timestamp(' + TIMESTAMP + '),', 'message = $$' + MESSAGE + '$$', 'WHERE id = (', '  SELECT id FROM ' + TABLE_NAME, '  WHERE isUsing = False', '  OR timestamp < to_timestamp(' + (TIMESTAMP - 90) + ') LIMIT 1', ')'].join(' ');
            ...
            sendSQL(UPDATE_ROW).then(function (result) {
              ...
              var GET_ID = ['SELECT id FROM ' + TABLE_NAME, 'WHERE hash = $$' + HASH + '$$', 'AND isUsing = True'].join(' ');
          ...
          if (CODE === 'GET') {
            var QUERY_FOR_GET = ['SELECT message FROM ' + TABLE_NAME, 'WHERE id = $escape$' + MESSAGE + '$escape$', 'AND hash = $escape$' + HASH + '$escape$'].join(' ');
